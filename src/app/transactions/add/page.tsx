...
export default function AddTransactionPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50" onKeyDown={handleKeyDown}>
      <header className="bg-zinc-900 border-b border-zinc-800">
        <div className="max-w-6xl mx-auto px-4 py-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white">Quick Add Transaction</h1>
            <p className="text-sm text-zinc-400 mt-1">
              Keyboard-friendly manual entry
            </p>
          </div>
          <button
            type="button"
            onClick={() => setShowShortcuts((prev) => !prev)}
            className="px-3 py-1.5 text-sm bg-zinc-800 hover:bg-zinc-700 rounded border border-zinc-700 text-zinc-400 transition-colors"
          >
            ? Shortcuts
          </button>
        </div>
